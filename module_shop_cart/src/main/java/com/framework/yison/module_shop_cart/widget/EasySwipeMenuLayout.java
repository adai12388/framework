package com.framework.yison.module_shop_cart.widget;
...
import com.framework.yison.module_shop_cart.R;
...
public class EasySwipeMenuLayout extends ViewGroup {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //1、获取配置的属性值
        TypedArray typedArray = context.getTheme().obtainStyledAttributes(attrs, R.styleable.cart_EasySwipeMenuLayout, defStyleAttr, 0);

        try {
            int indexCount = typedArray.getIndexCount();
            for (int i = 0; i < indexCount; i++) {
                int attr = typedArray.getIndex(i);
                if (attr == R.styleable.cart_EasySwipeMenuLayout_cart_leftMenuView) {
                    mLeftViewResID = typedArray.getResourceId(R.styleable.cart_EasySwipeMenuLayout_cart_leftMenuView, -1);
                } else if (attr == R.styleable.cart_EasySwipeMenuLayout_cart_rightMenuView) {
                    mRightViewResID = typedArray.getResourceId(R.styleable.cart_EasySwipeMenuLayout_cart_rightMenuView, -1);
                } else if (attr == R.styleable.cart_EasySwipeMenuLayout_cart_contentView) {
                    mContentViewResID = typedArray.getResourceId(R.styleable.cart_EasySwipeMenuLayout_cart_contentView, -1);
                } else if (attr == R.styleable.cart_EasySwipeMenuLayout_cart_canLeftSwipe) {
                    mCanLeftSwipe = typedArray.getBoolean(R.styleable.cart_EasySwipeMenuLayout_cart_canLeftSwipe, true);
                } else if (attr == R.styleable.cart_EasySwipeMenuLayout_cart_canRightSwipe) {
                    mCanRightSwipe = typedArray.getBoolean(R.styleable.cart_EasySwipeMenuLayout_cart_canRightSwipe, true);
                } else if (attr == R.styleable.cart_EasySwipeMenuLayout_cart_fraction) {
                    mFraction = typedArray.getFloat(R.styleable.cart_EasySwipeMenuLayout_cart_fraction, 0.5f);
                }
            }
