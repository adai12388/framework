...
/**
 * @author liubp 时间 ：2015-10-29 下午12:04:02 作用
...
public class DragLayout extends ViewGroup {
    ...
    private List<ShowPageNotifier> mPageListeners = new ArrayList<>(); // 手指松开是否加载下一页的notifier
    ...
    public void animTopOrBottom(View releasedChild, float yvel) {
        ...
        if (releasedChild == frameView1) {
            ...
            if (yvel < -VEL_THRESHOLD
                ...
                // 下一页可以初始化了
                for (ShowPageNotifier pageListener : mPageListeners) {
                    pageListener.onDragNext();
                }
            }
        } else {
            // 拖动第二个view松手
    ...
    public boolean onTouchEvent(MotionEvent e) {
        try {
            ...
            mDragHelper.processTouchEvent(e); // 该行代码可能会抛异常，正式发布时请将这行代码加上try
            // catch
        } catch (Exception e2) {
        }
        return true;
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        } else if (frameView2.getTop() == 0) {
            frameView1.layout(l, 0, r, b - t);
            frameView2.layout(l, 0, r, b - t);
            viewHeight = frameView1.getMeasuredHeight();
            frameView1.offsetTopAndBottom(-viewHeight);
        } else {
    ...
    public void addPageListener(@NonNull ShowPageNotifier pageListener) {
        this.mPageListeners.add(pageListener);
    }

    public void removePageListener(@NonNull ShowPageNotifier pageListener) {
        this.mPageListeners.remove(pageListener);
    }

    public interface ShowPageNotifier {
    ...
    public void selectNextPageSmooth() {
        if (mDragHelper.smoothSlideViewTo(frameView1, 0, -viewHeight)) {
            ViewCompat.postInvalidateOnAnimation(this);
        }
    }

    public void selectNextPage() {
//        frameView1.layout(frameView1.getLeft(), -frameView1.getHeight(), frameView1.getRight(), 0);
//        frameView2.layout(frameView2.getLeft(), 0, frameView2.getRight(), frameView2.getHeight());
        frameView1.setTop(-viewHeight);
        frameView2.setTop(0);
        requestLayout();
        for (ShowPageNotifier pageListener : mPageListeners) {
            pageListener.onDragNext();
    ...
    public void selectLastPageSmooth() {
        if (mDragHelper.smoothSlideViewTo(frameView2, 0, viewHeight)) {
            ViewCompat.postInvalidateOnAnimation(this);
        }
    }
